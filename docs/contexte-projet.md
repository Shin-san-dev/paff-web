...
L’atelier privé existant et ses résultats historiques restent consultables, avec les contrôles d’accès habituels. Aucun résultat d’apprentissage récent n’est publié, aucune statistique de joueur créée. Les anciens documents de campagnes conservés dans `docs/equilibrage/` sont historiques ; leurs commandes s’exécutent désormais dans le dépôt simulateur.
