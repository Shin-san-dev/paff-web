...
Publication autorisée par Nicolas le 15 septembre : le commit fonctionnel `44d82df` a été intégré à `main` et poussé sur GitHub. Les fonctions sont déployées sur **tough-gecko-249** ; la validation de schéma est réussie, sans suppression d’index. `catalogue2026:apply --prod` a mis à jour 20 fiches (3 profils et les métadonnées), sans création ni archivage. Les lectures de production confirment Bande de Gobelins 2 dés, Archers Gobelins 1 R / 2 dés et Epéistes Sephosiens 3 DT ; `health:check --prod` répond « operational ». Les anciennes parties gardent leurs profils copiés.

L’interface est publiée automatiquement par Vercel depuis `main`, avec son contrôle `npm run check`. Site : [paff-web.vercel.app](https://paff-web.vercel.app). Le statut du déploiement associé à chaque commit est consultable dans GitHub. Aucun nouveau scénario de partie ni changement des comptes/badges n’est exécuté en production pour cette publication.
