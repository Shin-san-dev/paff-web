...
- **Interface :** l’atelier privé du site garde ses anciens rapports et relectures. Le laboratoire d’apprentissage est dans le dépôt séparé, accessible localement sur le port 5181. Les statistiques concernent les bots uniquement.
...
Les fichiers ignorés laissés dans l’ancien dossier `analysis/` servent de sauvegarde locale et ne sont ni testés ni publiés avec le site. Aucun nettoyage destructif demandé. Le nouveau dépôt ne contient ni `.env.local`, ni configuration Convex/Vercel, ni données de joueurs.
